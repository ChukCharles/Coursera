...
void PrintStats(vector<Person> persons) {
	...
	auto it = partition( persons.begin(), persons.end(), []( const Person& p ) { return p.gender == Gender::FEMALE; });
	cout << "Median age for females = " << ComputeMedianAge( persons.begin(), it ) << endl;
	it = partition( persons.begin(), persons.end(), []( const Person& p ) { return p.gender == Gender::MALE; });
	cout << "Median age for males = " << ComputeMedianAge( persons.begin(), it ) << endl;
	it = partition( persons.begin(), persons.end(), []( const Person& p ) { return ( p.gender == Gender::FEMALE && p.is_employed ); });
	...
	it = partition( persons.begin(), persons.end(), []( const Person& p ) { return ( p.gender == Gender::MALE && p.is_employed ); });
