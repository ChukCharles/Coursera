...
public:
	explicit Unit(geo2d::Point position) : _area(position) {}
	...
	geo2d::Point area() const { return _area; };
private:
	geo2d::Point _area;
};
...
public:
	explicit Building(geo2d::Rectangle geometry) : _area(geometry) {}
	bool Collide(const GameObject& that) const;
	...
	geo2d::Rectangle area() const { return _area; }
private:
	geo2d::Rectangle _area;
};

class Tower : public GameObject {
public:
	explicit Tower(geo2d::Circle geometry) : _area(geometry) {}
	bool Collide(const GameObject& that) const;
	...
	geo2d::Circle area() const { return _area; }
private:
	geo2d::Circle _area;
};
...
public:
	explicit Fence(geo2d::Segment geometry) : _area(geometry) {}
	bool Collide(const GameObject& that) const;
	...
	geo2d::Segment area() const { return _area; }
private:
	geo2d::Segment _area;
};
...
bool Unit::CollideWith(const Unit& that) const {
	return geo2d::Collide(_area, that.area());
}
...
int main() {
	...
	//Unit u1({ 3, 3 }), u2({ 3, 5 }), u3({ 3, 3 });
	//cout << Collide(u1, u2) << endl
	//	<< Collide(u1, u3) << endl;
	return 0;
}
