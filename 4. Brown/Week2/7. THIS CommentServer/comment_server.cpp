...
public:
	...
	friend ostream& operator << (ostream& output, const HttpResponse& resp) {
		...
		//headers
		for (const string& s : resp.header()) {
			output << s << endl;
		}
		//content
	...
	HttpCode code() const { return _code; }
	const vector<string>& header() const { return _headers; }
	...
	HttpResponse ServeRequest(const HttpRequest& req) {
		...
		if (req.method == "POST") {
			if (req.path == "/add_user") {
				...
				return result.SetContent(response);
			}
				...
				if (banned_users.count(user_id) == 0) {
					...
					return result.SetCode(HttpCode::Found).AddHeader("Location","/captcha");
				}
			}
				...
				return result.SetCode(HttpCode::NotFound);
			}
		}
			...
			if (req.path == "/user_comments") {
				...
				//cout << "Comments: " << response << endl;
				return result.SetContent(response);
			}
			else if (req.path == "/captcha") {
				return result.SetContent("What's the answer for The Ultimate Question of Life, the Universe, and Everything?");
			}
			else {
				return result.SetCode(HttpCode::NotFound);
			}
...
void Test(CommentServer& srv, const HttpRequest& request, const ParsedResponse& expected) {
	stringstream ss;
	HttpResponse resp1 = srv.ServeRequest(request);
	//cout << resp1;
	ss << resp1;
	ParsedResponse resp;
	ss >> resp;
