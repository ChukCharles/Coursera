...
public:
    ...
    class IndexProxy {
    public:
        IndexProxy(Polynomial& p, size_t degree)
            : poly_(p), degree_(degree)
        {}
        ...
        IndexProxy& operator =(const T& val) {
            if (val == 0 && degree_ > poly_.Degree()) return *this;
            if (degree_ > poly_.Degree()) {
                poly_.coeffs_.insert(poly_.coeffs_.end(), degree_ - poly_.coeffs_.size() + 1, 0);
            }
...
//template <typename T>
//std::ostream& operator <<(std::ostream& out, const IndexProxy p) {
//    out << p();
//    return out;
//}
