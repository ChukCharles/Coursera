...
public:
    ...
    explicit ConcurrentMap(size_t bucket_count)
        : mv(bucket_count), data(bucket_count), size(bucket_count)
    {}

    //Access operator[](const K& key) {
    //    int index = (key % size);
    //    index = (index < 0) ? -index : index;
    //    return { lock_guard(mv[index]), data[index][key] };
    //}
    ...
    ReadAccess At(const K& key) const {
        int index = (hasher(key) % size);
        index = (index < 0) ? -index : index;
        return { lock_guard(mv[index]), data[index].at(key) };
    }

    bool Has(const K& key) const {
        int index = (hasher(key) % size);
        index = (index < 0) ? -index : index;
        lock_guard<mutex> g(mv[index]);
        if (data[index].count(key) > 0) return true;
        else return false;
    }
    ...
    MapType BuildOrdinaryMap() const {
        ...
        for (int i = 0; i < mv.size(); ++i) {
            lock_guard<mutex> g(mv[i]);
            res.insert(data[i].begin(), data[i].end());
        }
        return res;
    }
