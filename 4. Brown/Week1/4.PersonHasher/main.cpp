...
struct AddressHasher {
    size_t operator() (const Address& adr) const {
        size_t ch = shash(adr.city);
        size_t sh = shash(adr.street);
        size_t bh = ihash(adr.building);
        int n = 13;
        return ch * n * n + sh * n + bh;
    }
    hash<string> shash;
    hash<int> ihash;
};
...
struct PersonHasher {
    ...
    size_t operator() (const Person& p) const {
        size_t nh = shash(p.name);
        size_t hh = ihash(p.height);
        size_t wh = dhash(p.weight);
        size_t ah = ahash(p.address);
        int n = 13;
        size_t result = nh * n * n * n + hh * n * n + wh * n + ah;
        return result;
    }
};
...
void TestSmoke() {
    ...
    points.push_back({ "John", 180, 82.5, {"London", "Baker St", 221} });
    points.push_back({ "Sherlock", 190, 75.3, {"London", "Baker St", 221} });
