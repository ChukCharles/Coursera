...
public:
    ...
    AirportCounter() {
        for( int i = 0; i < static_cast<uint32_t>(TAirport::Last_); ++i) {
            //_data[TAirport(i)] = 0;
            _counter[i] = make_pair(TAirport(i), 0);
        }
    }
    ...
    AirportCounter(TIterator begin, TIterator end) {
        ...
        while (begin != end) {
            //_data[*begin]++;
            _counter[static_cast<uint32_t>(*begin)].second += 1;
            begin++;
        }
    }
    ...
    size_t Get(TAirport airport) const {
        return _counter[static_cast<uint32_t>(airport)].second;
    }

    // добавить данный элемент

    void Insert(TAirport airport) {
        _counter[static_cast<uint32_t>(airport)].second++;
    }

    // удалить одно вхождение данного элемента

    void EraseOne(TAirport airport) {
        if (_counter[static_cast<uint32_t>(airport)].second > 0) {
            _counter[static_cast<uint32_t>(airport)].second--;
        }
    }
    ...
    void EraseAll(TAirport airport) {
        _counter[static_cast<uint32_t>(airport)].second = 0;
    }
...
private:
    //Items _data;
    //array<size_t, static_cast<uint32_t>(TAirport::Last_)> _data;
    Items _counter;
};
...
void TestMoscow() {
    ...
    //static_cast<uint32_t>(SomeCityAirport::Last_)
